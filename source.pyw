                ...
                while firmware != "":
                    try:
                        ...
                        if len(ipsw_id) == 4:
                            ...
                            (
                                int(re.sub(r"[^\d]*|\d*$", "", self.device_id())) < 7
                                and len(ipsw_id) > 5
                            )
                            or ("P3" == ipsw_id[2])
                            or (len(ipsw_id) == 6)
                    ...
                    if bm.status_code == 200:
                        ...
                        (download_path() / f"{self.device_id()}").mkdir(exist_ok=True)

                        (
                            download_path()
                            / f"{self.device_id()}/{ios_version}-{ios_build_id}.plist"
                        ).write_bytes(bm.content)

                        messagebox.showinfo(
                            "Success",
                            f"Downloaded {ios_version}-{ios_build_id}.plist",
                        )
...
if __name__ == "__main__":
    # Download devices.json file if it doesn't exist
